                ...
                <div class="pt-4 px-6">
                        ...
                        <div class="flex">
                            <select id="category" class="appearance-none block pl-4 pr-8 py-3 mb-2 text-sm bg-white border rounded" name="category_id">
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" @if ($category->id == old('category_id', $blog->category->id))
                                        selected                                        
                    ...
                    <div class="mb-6">
                        ...
                        <select id="js-pulldown" class="mr-6 w-full" name="cats[]" multiple>
                            <option value="">選択しな</option>
                            @foreach ($cats as $cat)
                                <option value="{{ $cat->id }}" @if (in_array($cat->id, old('cats', $blog->cats->pluck('id')->all())))
                                    selected
